...
void Simulation::init_particles( const double size, const int part_seed) {
    ...
    // int num_parts = this->D_parts.size();
    ...
    //     D_parts[i]=Particle(x, y, z, vx, vy, vz, m, charge);
    // }
...
__host__ __device__ void Simulation::simulate_one_step(AbstractForce* force,const int num_parts,const double size) {
    // Compute Forces
    ...
    // #endif
    // for (int i = 0; i < num_parts; ++i) {
    //     static_cast<Particle>(this->D_parts[i]).ax  = 0.;
    //     for (int j = 0; j < num_parts; ++j) {
    //         force->force_application(D_parts[i], D_parts[j]);
    //     }
    // }
    // #ifdef _OPENMP
	// #pragma omp barrier
    ...
    // #endif
    // for (int i = 0; i < num_parts; ++i) {
    //      static_cast<Particle>(D_parts[i]).move(size);
    // }
};
